...
class AIRTPRuntime:
    ...
    async def execute(self, text):
        ...
        self.logical_buffer = ""
        self.state = RuntimeState.ACTIVE
        await self.executeRuntime(envelope)
        self.logical_buffer = self.logical_buffer.replace("<END>", "")
        self.logical_buffer = self.logical_buffer.rstrip()
        if not self.logical_buffer.endswith("\n"):
            self.logical_buffer += "\n"
...
if __name__ == "__main__":
    main()
trey@yoga:~/tool_ideas/AIRTP$ ls
AIRTP_openai_contract.json  docs         no_way.b64.png         scripts
AIRTP_output_contract.json  examples     no_way.png             test_index.html
AIRTP.py                    index.html   question.prompt        test.prompt
blown_away.png              LICENSE      README.md              this.prompt
CNAME                       mailmap.txt  refactor_AIRTP.prpmpt  WHAT.png
trey@yoga:~/tool_ideas/AIRTP$ vi question.prompt 
trey@yoga:~/tool_ideas/AIRTP$ vi AIRTP.py 
trey@yoga:~/tool_ideas/AIRTP$ vi question.prompt 
trey@yoga:~/tool_ideas/AIRTP$ vi AIRTP.py 

[1]+  Stopped                 vi AIRTP.py
trey@yoga:~/tool_ideas/AIRTP$ vi AIRTP.py 
trey@yoga:~/tool_ideas/AIRTP$ vi AIRTP.py 
trey@yoga:~/tool_ideas/AIRTP$ cat AIRTP.py 
#!/usr/bin/env python3
